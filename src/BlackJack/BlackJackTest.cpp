...
* @file       Tests.cpp
*
* @brief      Test routines for BlackJack.
*
...
Test_GenerateGroupParams()
{
	...
	for (count = 0; count < 1; count++) {
		...
		try {
			group = deriveIntegerGroupParams(calculateSeed(GetTestModulus(), "test", BLACKJACK_DEFAULT_SECURITYLEVEL, "TEST GROUP"), pLen, qLen);
...
Test_ParamGen()
{
	...
	try {
		...
		Params testParams(GetTestModulus(),BLACKJACK_DEFAULT_SECURITYLEVEL);
...
Test_RunAllTests()
{
	printf("BlackJack v%s self-test routine\n", BLACKJACK_VERSION_STRING);
	...
	// Run through all of the BlackJack tests
