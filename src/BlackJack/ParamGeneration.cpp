...
///
/// \brief      Parameter manipulation routines for the BlackJack cryptographic
///             components.
///
...
namespace libBlackJack {

/// \brief Fill in a set of BlackJack parameters from a modulus "N".
...
///
/// \throws         BlackJackException if the process fails
///
...
/// be known to any party, or the security of BlackJack is
...
CalculateParams(Params &params, Bignum N, string aux, uint32_t securityLevel)
{
	...
	if (NLen < 1023) {
		throw BlackJackException("Modulus must be at least 1023 bits");
	}

	// Verify that "securityLevel" is  at least 80 bits (minimum).
	if (securityLevel < 80) {
		throw BlackJackException("Security level must be at least 80 bits.");
	}
	...
	*pLen = *qLen = 0;
	...
	if (securityLevel < 80) {
		...
		*pLen = 3072;
		...
		throw BlackJackException("Security level not supported.");
	}

	if (*pLen > maxPLen) {
		throw BlackJackException("Modulus size is too small for this security level.");
	}
}
...
deriveIntegerGroupParams(uint256 seed, uint32_t pLen, uint32_t qLen)
{
	...
	if ((uint32_t)(result.modulus.bitSize()) < pLen ||          // modulus is pLen bits long
		...
		throw BlackJackException("Group parameters are not valid");
	}
...
deriveIntegerGroupFromOrder(Bignum &groupOrder)
{
	...
	throw BlackJackException("Too many attempts to generate Schnorr group.");
}
                              ...
                              uint256 *resultPseed, uint256 *resultQseed)
{
	...
	if (qLen > (sizeof(seed)) * 8) {
		...
		// throw BlackJackException("Seed is too short to support the required security level.");
	}

#ifdef BLACKJACK_DEBUG
	...
	*resultGroupOrder = generateRandomPrime(qLen, seed, &qseed, &qgen_counter);
	...
	// Throw an exception.
	throw BlackJackException("Unable to generate a prime modulus for the group");
}
...
/// \throws                             A BlackJackException if error.
///
...
calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, Bignum modulus, Bignum groupOrder, uint32_t index)
{
	...
	if (index > 255) {
		throw BlackJackException("Invalid index for group generation");
	}
	...
	throw BlackJackException("Unable to find a generator, too many attempts");
}
                    ...
                    uint32_t *prime_gen_counter)
{
	...
	if (primeBitLen < 2) {
		throw BlackJackException("Prime length is too short");
	}
	...
	if (primeBitLen < 33) {
		...
		throw BlackJackException("Unable to find prime in Shawe-Taylor algorithm");
	...
	throw BlackJackException("Unable to generate random prime (too many tests)");
}
